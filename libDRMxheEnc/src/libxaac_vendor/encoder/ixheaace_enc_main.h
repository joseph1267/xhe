...
/* DRM_XHE_VENDOR_PATCH: pv_mem_ctx passed to free hook */
    ...
    if (pstr_output_config->malloc_count > 0) {                                       \
      for (idx = pstr_output_config->malloc_count - 1; idx >= 0; idx--) {             \
        if (pstr_output_config->arr_alloc_memory[idx]) {                              \
          pstr_output_config->free_xheaace(pstr_output_config->pv_mem_ctx,            \
                                           pstr_output_config->arr_alloc_memory[idx]); \
        }                                                                             \
      }                                                                               \
