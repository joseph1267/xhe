...
typedef struct {
  ...
  /* DRM_XHE_VENDOR_PATCH: pv_mem_ctx cookie threaded through the allocator
     hooks for multi-instance safety (fdk-aac libDRMxheEnc integration). */
  pVOID (*malloc_xheaace)(pVOID, UWORD32, UWORD32);
  VOID (*free_xheaace)(pVOID, pVOID);
  pVOID pv_mem_ctx;
